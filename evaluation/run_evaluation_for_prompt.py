...
from utils.io import (
    load_extraction_jsonl_for_prompt, load_label_csv, save_evaluation_csv_for_prompt,
...
def run_evaluation_for_prompt(round_mark: str, dataset: str, model_abbr: str, model_name: str, prompt_strategy_type: str, eval_fields: list, eval_methods: list):
    print(f"⚙️ Running Evaluation for: round_time={round_mark}, model_abbr={model_abbr}, model_name={model_name}, dataset={dataset}, prompt={prompt_strategy_type}, eval_methods={eval_methods}")
    ...
    # evaluation_round_mark = "pmpt"
    evaluation_round_time = evaluation_round_time[:8]
    evaluation_round_mark = evaluation_round_time

    # Load input data
    input_file_name = f"{dataset}/{round_mark}-{model_abbr}-{model_name}-{prompt_strategy_type}-resp.jsonl"
    
    df_extract = load_extraction_jsonl_for_prompt(round_mark, dataset, model_abbr, model_name, prompt_strategy_type)
    ...
    for method in eval_methods:
        if method == "EM":
            ...
            output_file = save_evaluation_csv_for_prompt(result, evaluation_round_mark, dataset, model_abbr, model_name, prompt_strategy_type, "em")
            output_files.append(output_file)
        elif method == "TK":
            result = token_level(df_extract, df_label, eval_fields)
            output_file = save_evaluation_csv_for_prompt(result, evaluation_round_mark, dataset, model_abbr, model_name, prompt_strategy_type, "tk")
            output_files.append(output_file)
        elif method == "BS":
            result = bert_score(df_extract, df_label, eval_fields)
            output_file = save_evaluation_csv_for_prompt(result, evaluation_round_mark, dataset, model_abbr, model_name, prompt_strategy_type, "bs")
            output_files.append(output_file)
        else:
...
if __name__ == "__main__":
    ...
    for model_abbr, model_name in model_dict.items():
        for dataset in ["aws"]:
            for prompt_strategy_type in ['basic-fs', 'basic-zs', 'categ-zs', 'cot-zs', 'full-fs', 'full-zs']:
                run_evaluation_for_prompt( 
                    round_mark="pmpt",  # Prompt extraction mark 
                    dataset=dataset,
                    model_abbr=model_abbr,
                    model_name=model_name,
                    prompt_strategy_type=prompt_strategy_type,
