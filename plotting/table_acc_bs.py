...
prompt_lst = ['0', '1']
eval_method = 'bs'
plot_fields = ['user_symptom', 'root_cause']
...
plot_round_mark = 'table_bs'

# Create bit-level accuracy tables for each dataset
if mode == 'SINGLE':
    print(f"\n{'='*60}")
    ...
    for dataset in dataset_lst:
        print(f"\n{'='*60}")
        print(f"CREATING BERT SCORE TABLE FOR DATASET: {dataset.upper()}")
        print(f"{'='*60}")
        ...
        print(f"\nComprehensive BERT Score Table - {dataset.upper()}")
        print("-" * 120)
        ...
        comprehensive_csv_path = f'{output_dir}/{plot_round_mark}_{mode.lower()}_{dataset}.csv'
        df_comprehensive.to_csv(comprehensive_csv_path, index=False)
        print(f"\nSaved comprehensive BERT score table to: {comprehensive_csv_path}")
        ...
        comprehensive_latex_path = f'{output_dir}/{plot_round_mark}_{mode.lower()}_{dataset}.tex'
        with open(comprehensive_latex_path, 'w') as f:
            f.write(comprehensive_latex_table)
        print(f"Saved comprehensive BERT score LaTeX table to: {comprehensive_latex_path}")
    ...
    print(f"\n{'='*80}")
    print(f"CREATING COMBINED MULTI-DATASET BERT SCORE TABLE")
    print(f"{'='*80}")
    ...
    print(f"\nComprehensive Combined BERT Score Table - All Datasets")
    print("-" * 160)
    ...
    print(f"\nSaved comprehensive combined BERT score table to: {comprehensive_combined_csv_path}")
    
    # Generate LaTeX format for comprehensive combined table
    print("\nComprehensive Combined Table LaTeX Format:")
    print("-" * 80)
    comprehensive_combined_latex_table = df_comprehensive_combined.to_latex(float_format='{:.2f}'.format, index=False)
    # print(comprehensive_latex_table)
    ...
    print(f"Saved comprehensive combined BERT score LaTeX table to: {comprehensive_combined_latex_path}")
...
print("\n" + "="*80)
print("ALL BERT SCORE TABLES CREATED SUCCESSFULLY!")
print("="*80)
