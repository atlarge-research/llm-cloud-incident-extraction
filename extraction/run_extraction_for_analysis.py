'''
Run extraction for analysis
(i) Extract from 2_clean_data
(ii) Gemini 2.5 few-shot for AWS, Gemini 2.0 few-shot for AZURE
'''
...
from utils.io import (
    load_csv, parse_response_json, save_response_jsonl_for_analysis, save_model_perf_csv_for_analysis,
...
def run_extraction_for_analysis(dataset: str, model_abbr: str, model_name: str, prompt_type: str, round_mark: str = 'anl'):
    print(f"🔍 Running extraction for analysis: dataset={dataset}, model_abbr={model_abbr}, model_name={model_name}, prompt_type={prompt_type}")
    ...
    # Load input data
    input_file_name = f"{dataset}.csv"
    df = load_csv("data/2_clean_data", input_file_name)
    ...
    for i, row in df.iterrows():
        try:
            ...
            perf_file_name = save_model_perf_csv_for_analysis(
            ...
            print(response_json)
            output_file_name = save_response_jsonl_for_analysis(response_json, round_time, dataset, model_abbr, model_name, prompt_type)
            ...
            save_response_jsonl_for_analysis(response_json, round_time, dataset, model_abbr, model_name, prompt_type)
