...
from utils.io import (
    load_csv, parse_response_json, save_response_jsonl_for_prompt, save_model_perf_csv_for_prompt,
...
def get_prompt_strategy_path(dataset: str, prompt_strategy_type: str) -> str:
    """Build the strategy prompt file path dynamically."""
    return f"prompts/strategies/prompt_{prompt_strategy_type}.txt"
...
def run_extraction_for_prompt(dataset: str, model_abbr: str, model_name: str, prompt_strategy_type: str, round_mark: str = 'pmpt'):
    print(f"🔍 Running extraction: dataset={dataset}, model_abbr={model_abbr}, model_name={model_name}, prompt_strategy_type={prompt_strategy_type}")

    # Generate unique round_time for this extraction run
    round_time = generate_round_time()
    print(f"🕐 Extraction round time: {round_time}")

    # round_time = round_mark
    ...
    template_path = get_prompt_strategy_path(dataset, prompt_strategy_type)
    ...
    for i, row in df.iterrows():
        try:
            ...
            perf_file_name = save_model_perf_csv_for_prompt(
                round_time, dataset, model_abbr, model_name, prompt_strategy_type, 
                token_input, token_output, latency
            )

            response_json = parse_response_json(response_text)
            print(response_json)
            output_file_name = save_response_jsonl_for_prompt(response_json, round_time, dataset, model_abbr, model_name, prompt_strategy_type)
            ...
            save_response_jsonl_for_prompt(response_json, round_time, dataset, model_abbr, model_name, prompt_strategy_type)
